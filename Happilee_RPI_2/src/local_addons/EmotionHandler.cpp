...
void EmotionHandler::setup(int initialMemory,int _lowMemoryAmount,int _malfunctionTimerLength)
{
    ...
    shaderContainer.allocate(ofGetWidth(),ofGetHeight());
    
    shaderContainer.begin();
    ofClear(0);
    shaderContainer.end();
    
    shaderWobble.load("shaderExample");
...
void EmotionHandler::update()
{
    ...
    if (hpState == HAPPILEE_MALFUNCTION) {
        interferance.setFx(OFXPOSTGLITCH_GLOW, true);
...
void EmotionHandler::drawImages()
{
   ...
   // shaderWobble.begin();
   // shaderWobble.setUniformTexture("tex0", faceTexture.getTextureReference(), faceTexture.getTexture().getTextureData().textureID);
   // shaderWobble.setUniform1f("time", ofGetElapsedTimef());
   // shaderWobble.setUniform2f("resolution", ofGetWidth(), ofGetHeight());
   // shaderWobble.setUniform1f("rand",ofRandom(0.00,1.00));
...
void EmotionHandler::drawEmotions()
{
	...
	//ofScale(1,-1);    
int x = (offsetX*4)+receivingEmotionImage.getWidth();
/*    wordsContainer.getTexture().getTextureData().bFlipTexture = true; 
    wordsContainer.begin();
    ofClear(0, 0, 0, 0);
    ofPushStyle();
    ofPushMatrix();
...
//    interferance.setFx(OFXPOSTGLITCH_CR_GREENINVERT,true);
//    interferance.setFx(OFXPOSTGLITCH_CR_GREENINVERT,false);
...
void EmotionHandler::setHappileeState(HAPPILEE_STATE state)
{
    ...
    if (hpState == HAPPILEE_GREEN) {
        interferance.setFx(OFXPOSTGLITCH_INVERT,false);
...
{
    setImage(neutralEmotionID, 0);
    setHappileeState(HAPPILEE_GREEN);
    malfunctionedLatch = false;
    winningLatch = false;
    welcomeSound.play();
    cout << str << endl;
